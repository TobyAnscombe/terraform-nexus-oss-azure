#!/usr/bin/env bash
# Configure Nexus directly via its public URL — use this when Cloudflare is
# not blocking curl (Bot Fight Mode bypassed or WAF rule in place).
#
# Handles EULA, password bootstrap, and full Phase 2 configuration
# (routing rules, repos, roles, anonymous access) in one pass.
#
# Required env vars:
...
: "${NEXUS_URL:?NEXUS_URL is required (e.g. https://nexus.example.com)}"
CURRENT_PASSWORD="${CURRENT_PASSWORD:-admin123}"

BASE="${NEXUS_URL%/}"

# Try with $1 password, fall back to $2.
try_pw() {
  local method="$1" path="$2" pw1="$3" pw2="$4"
  shift 4
  local code
  code=$(curl -s -u "admin:${pw1}" -X "$method" "${BASE}${path}" "$@" -o /tmp/nx_out -w '%{http_code}')
  if [[ "$code" != 2* ]]; then
    code=$(curl -s -u "admin:${pw2}" -X "$method" "${BASE}${path}" "$@" -o /tmp/nx_out -w '%{http_code}')
  fi
  if [[ "$code" != 2* ]]; then
    echo "  FAIL($code) $method $path: $(head -c 300 /tmp/nx_out 2>/dev/null)"
    return 1
  fi
...
post_or_put() {
  ...
  code=$(curl -s -u "admin:${NEW_PASSWORD}" -X POST "${BASE}${cp}" \
    -H 'Content-Type: application/json' -d "$body" \
    -o /tmp/nx_out -w '%{http_code}')
  if [ "$code" = "409" ] || [ "$code" = "422" ] || [ "$code" = "400" ]; then
    code=$(curl -s -u "admin:${NEW_PASSWORD}" -X PUT "${BASE}${up}" \
      -H 'Content-Type: application/json' -d "$body" \
      -o /tmp/nx_out -w '%{http_code}')
  fi
  if [[ "$code" != 2* ]]; then
    echo "  FAIL($code) $cp: $(head -c 300 /tmp/nx_out 2>/dev/null)"; return 1
  fi
  echo "  OK($code) $cp"
}
...
do_put() {
  ...
  code=$(curl -s -u "admin:${NEW_PASSWORD}" -X PUT "${BASE}${path}" \
    -H 'Content-Type: application/json' -d "$body" \
    -o /tmp/nx_out -w '%{http_code}')
  if [[ "$code" != 2* ]]; then
    echo "  FAIL($code) PUT $path: $(head -c 300 /tmp/nx_out 2>/dev/null)"; return 1
  fi
  echo "  OK($code) PUT $path"
}

echo "=== Nexus bootstrap + configuration ==="
echo "Target: $BASE"

# ── 1. Accept EULA ────────────────────────────────────────────────────────────
echo "--- EULA"
try_pw PUT /service/rest/v1/system/eula "$CURRENT_PASSWORD" "$NEW_PASSWORD" \
  -H 'Content-Type: application/json' -d '{"accepted":true}'

# ── 2. Change admin password ──────────────────────────────────────────────────
echo "--- Admin password"
try_pw PUT /service/rest/v1/security/users/admin/change-password \
  "$CURRENT_PASSWORD" "$NEW_PASSWORD" \
  -H 'Content-Type: text/plain' -d "$NEW_PASSWORD"

# From here all calls use NEW_PASSWORD (already set above).
